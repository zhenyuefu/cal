...
from api.data import parcours_s3 as _parcours_s3_src
...
# 复制一份，避免运行期修改全局数据结构引发并发条件 / 冷热启动不一致
parcours_s3 = deepcopy(_parcours_s3_src)
...
def filter_cours(ue_group: dict, majour: str):
    # 动态扩展 OIP
    local_mapping = dict(parcours_s3)
    local_mapping["OIP"] = majour
    parcours_ue = filter_ue_parcours(ue_group, local_mapping)
    ...
    for ue_name, group_code in ue_group.items():
        try:
            compiled_patterns[ue_name] = re.compile(
                rf"T\w{{1,2}}(?:(?=\d)(?!{group_code})|\s*[Gg][Rr](?!{group_code}))"
            )
        except re.error:
    ...
    for parcour in parcours_ue.keys():
        for event in _iter_events_for_parcour(parcour):
            summary = event.get("summary", "") if IS_INDEX_MODE.get(parcour) else getattr(event, "summary", "")  # type: ignore
    ...
    ics_text = build_ics("M2 " + majour, collected_index_events)
    class _Wrapper:
